<template>
  <div id="content">
    <div ref="content" class="content-c">
      ...
      <p> 使用常规的 ajax axios fetch等传统上传手段，当在正在上传时，页面被卸载会导致数据丢失，进而上传失败。</p>
      <p> 为了解决这个问题，<code>navigator.sendBeacon</code> API 被引入，它可以在页面卸载时安全、可靠地发送数据。</p>
      ...
      <ol start="4">
        ...
        <li>
          <p>2. endBeacon 只能传送少量数据（64KB 以内），fetch 和 ajax 可以传输任意字节数据 </p>
        </li>
        <li>
          <p>3. sendBeacon 无法自定义请求头，fetch 和 ajax 可以定义任意请求头</p>
        </li>
        <li>
          <p>4. sendBeacon 只能传输特定的类型
          </p>
        </li>
        <li>
          <p>5. 如果处于危险的网络环境，或者开启了广告屏蔽插件 此请求将无效。</p>
        </li>
      </ol>
      <p> <strong>应用场景</strong></p>
      <ol start="9">
        <li>
          <p>1. 数据埋点统计( 多 )：在页面关闭或卸载时记录用户在线时间，pv( 页面访问量 ) uv( 独立访客数 )，以及错误日志上报 按钮点击次数。</p>
        </li>
        <li>
          <p>2. 发送用户反馈：用户意见、bug 报告等，以便进行产品优化和改进。</p>
        </li>
        <li>
          <p>3. 发送心跳包：发送心跳包，以保持与服务器的长连接，避免因为长时间没有网络请求而导致连接被关闭。</p>
        </li>
      </ol>
      <p> <strong>该API的使用demo</strong></p>
      <p> 按照 小满zs 的步骤来还是无法 传递对象。只能传递字符串</p>
      <h3>实例代码</h3>
      <highlightjs class="code" language='javascript' code="  const sendBtnEl = document.querySelector(&#39;.send&#39;)
  ...
  sendBtnEl.addEventListener(&#39;click&#39;, () =&gt; {
    ...
    // })
      ...
      <highlightjs class="code" language='javascript' code="// 引入express框架
