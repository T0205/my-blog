        ...
        <div class="forms">
          ...
          <div class="form switch-login left">
            ...
            <button class="btn-login"
              @click="(toLogin('.switch-login.left', '.form-input.right', '.form-input.left', '.switch-register.right'))">登录</button>
          </div>

          <div class="form form-input right">
            <h1>注册</h1>
            <input v-model="resgisterUser.username" type="text" placeholder="用户名">
            <input v-model="resgisterUser.pwd" type="password" placeholder="密码">
            <!-- 失去焦点时 触发判断两次密码是否一致的函数 -->
            <input v-model="resgisterUser.confirmPwd" @blur="comparePwd" type="password" placeholder="确认密码">
            <input v-model="resgisterUser.tel" type="text" placeholder="电话号码" @keydown.enter="registe">
          ...
          <div class="form form-input left">
            ...
            <input v-model="user.password" type="password" placeholder="密码" @keydown.enter="dologin">
            <p>忘记密码?</p>
            <button id="loginButton" class="btn-login" @click="dologin">登录</button>
          </div>
          ...
          <div class="form switch-register right">
            ...
            <button class="btn-register"
              @click="toRegister('.switch-login.left', '.form-input.right', '.form-input.left', '.switch-register.right')">注册</button>
          </div>
...
// 登录和注册界面的动画
function toLogin(classNameLeft: any, classNameRight: any, classNameBottom: any, ToRight: any) {
...
function toRegister(classNameLeft: any, classNameRight: any, classNameBottom: any, ToRight: any) {
  let ElLeft = document.querySelector(classNameLeft)
